...
agent fields ?
why extends stream
  to preserve nodejs Request trait
  can sending various event on various scenario including
    hanle multiple response on redirect
...
var resolve = require('url').resolve;//like path.resolve
...
//  using Gzip and Deflate/Inflate. It can be accessed using:
...
function Request(method, url) {
  Stream.call(this);// why stream
  var self = this;
  if ('string' != typeof url) url = format(url);
  this._agent = false; //todo: 
  this._formData = null;
  this.method = method;
  this.url = url;
  _initHeaders(this);
...
Request.prototype.query = function(val){
  ...
  extend(this.qs, val);// like jquery extend
  return this;
};
...
Request.prototype.pipe = function(stream, options){
  ...
  this.buffer(false);
  var self = this;
  this.end().req.on('response', function(res){//will be triggered repeatly on redirect response
    // redirect
    ...
    self.res = res;
    self._emitResponse();// sending out a response event
    ...
    res.on('end', function(){// since redirect rebind its end event to _callback, this will trigger it
      self.emit('end');
    });
  });
...
Request.prototype._redirect = function(res){
  ...
  // location
  url = resolve(this.url, url);// has to be same origin
...
Request.prototype.request = function(){
  ...
  // request, send request with nodejs http or https
